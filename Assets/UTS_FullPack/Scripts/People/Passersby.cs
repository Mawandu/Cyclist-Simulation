...
public class Passersby : MonoBehaviour
{
    ...
    public UTS_MovePath movePath { get; set; }
    ...
    private void SortTargetsInView<T>(IList<T> list, Collider[] targetsInViewRadius, string tag)
    {
        foreach(Collider c in targetsInViewRadius)
        {
            ...
            {
                if(c.transform.parent.transform.parent.name != transform.parent.transform.parent.name || c.GetComponent<UTS_MovePath>().w != movePath.w || c.GetComponent<UTS_MovePath>().forward != movePath.forward) continue;
            }
