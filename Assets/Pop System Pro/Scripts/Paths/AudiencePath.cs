    ...
    public override void SpawnPeople()
    {
        ...
        for (int wayIndex = 0; wayIndex < numberOfWays; wayIndex++)
        {
            ...
            for (int peopleIndex = 0; peopleIndex < peopleCount; peopleIndex++)
            {
                ...
                movePath.MyStart(wayIndex, GetRoutePoint((peopleIndex + 1) * segmentLen + randomShift, wayIndex, pCount, true, loopPath), "", loopPath, true, 0);
                ...
                rrr = Physics.RaycastAll(or = new Vector3(people.transform.position.x, people.transform.position.y + 10000, people.transform.position.z), Vector3.down, Mathf.Infinity);
